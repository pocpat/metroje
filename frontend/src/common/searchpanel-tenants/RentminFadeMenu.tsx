...
import * as React from 'react';
import { FormControl, FormHelperText, Select, MenuItem, SelectChangeEvent } from '@mui/material';
import { menuItemSelectStyle, formHelperTextSelectorStyle, selectInputGreyStyle } from './menuItemSelectStyle';    
import { RentminSelectLabelsProps } from '../../../types/props';

const RentminSelectLabels: React.FC<RentminSelectLabelsProps> = ({ rmin, setRmin }) => {
  const handleChange = (event: SelectChangeEvent<number>) => {
    setRmin(event.target.value as number);
  };

  return (
    <div>
      <FormControl sx={{ m: 1, minWidth: 120 }}>
        <FormHelperText sx={formHelperTextSelectorStyle}>Rent Min</FormHelperText>
        <Select
          value={rmin}
          onChange={handleChange}
          displayEmpty
...
export default RentminSelectLabels;
